(() => {
  ...
  function renderNav() {
    els.lessonNav.innerHTML = state.manifest.lessons.map((item, idx) => `
      <button class="lesson-chip ${idx === state.lessonIndex ? 'active' : ''}" data-lesson="${escapeHtml(item.id)}" type="button">
        <span>${String(idx + 1).padStart(2, '0')}</span>${escapeHtml(item.title)}
      </button>
    `).join('');
  ...
  function renderContent() {
    ...
    els.pageSubtitle.textContent = `Bài ${l.id} · ${l.subtitle || ''}`;
    ...
    els.examplesList.innerHTML = (l.examples || []).map((ex) => `
      <div class="example-row">
        <span class="example-label">${escapeHtml(ex.id)}</span>
        <div>
          <strong>${escapeHtml(ex.title || ex.id)}</strong>
          <p>${escapeHtml(ex.prompt || '')}</p>
          <p class="example-answer ${state.showAnswers ? '' : 'hidden'}"><strong>Đáp án:</strong> ${escapeHtml(ex.answer || '')}</p>
        </div>
      </div>
    `).join('');

    els.quizList.innerHTML = (l.quiz || []).map((q, idx) => `
      <div class="quiz-item" data-answer="${Number(q.answer)}">
        <p>${idx + 1}. ${escapeHtml(q.question)}</p>
        ${(q.options || []).map((opt, oi) => `<label><input type="radio" name="q-${l.id}-${idx}" value="${oi}"> ${escapeHtml(opt)}</label>`).join('')}
      </div>
    `).join('');
    els.quizResult.textContent = '';

    els.conceptGrid.innerHTML = (l.concepts || []).map((c) => `
      <article class="concept-card">
        <span class="concept-number">${escapeHtml(c.number)}</span>
    ...
    els.footerLesson.textContent = `Price Action Trainer · Lesson ${l.id}`;
    renderNav();
  }
  ...
  function lessonMarkers() {
    ...
    return (state.lesson.examples || []).map((ex) => ({
      time: ex.time,
      position: ex.position || 'aboveBar',
      color: ex.color || '#7aa7ff',
      shape: ex.shape || 'circle',
      text: state.showAnswers ? `${ex.id} · ${ex.title || ''}` : ex.id
    }));
  }

  function clearPriceLines() {
    state.priceLines.forEach((line) => {
      try { candles.removePriceLine(line); } catch (_) { /* noop */ }
    });
    state.priceLines = [];
  }
  ...
  function renderPriceLines() {
    ...
    (state.lesson.priceLines || [])
      .filter((line) => !line.answerOnly || state.showAnswers)
      .forEach((line) => {
        const apiLine = candles.createPriceLine({
          price: Number(line.price),
          color: line.color || '#52647d',
          lineWidth: 1,
          lineStyle: Number.isFinite(line.lineStyle) ? line.lineStyle : LightweightCharts.LineStyle.Dashed,
          axisLabelVisible: true,
          title: state.showAnswers || !line.answerOnly ? (line.title || '') : ''
        });
  ...
  function renderAnswers() {
    state.markersApi.setMarkers(lessonMarkers());
    renderPriceLines();
  ...
  async function loadLessonById(id, updateUrl = true) {
    ...
    const lesson = await fetchJson(`./lessons/${item.file}`);
  ...
  async function loadLive() {
    ...
    clearPriceLines();
    els.answerHint.classList.add('hidden');
    els.answerBtn.textContent = 'Bật đáp án';
    ...
    try {
      ...
      const fallback = await resolveCandles(state.lesson);
      setData(fallback);
      els.chartSource.textContent = `Không tải được Binance live; dùng snapshot bài học. (${err.message})`;
    }
    setModeButtons();
  }

  function gradeQuiz() {
    const items = [...els.quizList.querySelectorAll('.quiz-item')];
    if (!items.length) return;
    let score = 0;
    items.forEach((item, idx) => {
      const picked = item.querySelector(`input[name="q-${state.lesson.id}-${idx}"]:checked`);
      if (picked && Number(picked.value) === Number(item.dataset.answer)) score += 1;
    });
    els.quizResult.textContent = score === items.length
      ? `✓ ${score}/${items.length} — Tốt. Hãy quay lại chart và giải thích bằng evidence, không chỉ nhớ đáp án.`
      : `${score}/${items.length} — Chưa sao. Bật đáp án trên chart, đọc lại framework rồi thử giải thích từng điểm bằng candle + context.`;
  }
  ...
  els.lessonNav.addEventListener('click', (e) => {
    const btn = e.target.closest('[data-lesson]');
    if (btn) loadLessonById(btn.dataset.lesson).catch(console.error);
  });

  els.prevLessonBtn.addEventListener('click', () => {
    if (state.lessonIndex > 0) loadLessonById(state.manifest.lessons[state.lessonIndex - 1].id).catch(console.error);
  });

  els.nextLessonBtn.addEventListener('click', () => {
    if (state.lessonIndex < state.manifest.lessons.length - 1) loadLessonById(state.manifest.lessons[state.lessonIndex + 1].id).catch(console.error);
  });

  els.lessonModeBtn.addEventListener('click', () => {
    if (state.lesson) loadLessonById(state.lesson.id, false).catch(console.error);
  });
  ...
  init().catch((err) => {
    els.chartSource.textContent = `Lỗi khởi tạo khóa học: ${err.message}`;
    els.lessonTitle.textContent = 'Không tải được bài học';
    console.error(err);
  });
})();
